...
-- 1) 새로운 newbie 계정 만들기
CREATE USER 'newbie'@'%' IDENTIFIED BY  'newbie'; -- 'localhost' 대신 '%'를 쓰면 외부 ip로 접속 가능하다.
...
GRANT ALL PRIVILEGES ON menudb.* TO 'newbie'@'%';	-- menu에 대한 모든 권한 부여

SHOW GRANTS FOR 'newbie'@'%';

-- 3) 새로운 접속기 생성 후 접속하고 데이터베이스 활용하기
-- 좌측 상단의 home 버튼을 눌러 newbie 계정 접속기를 만들어 접속하고 database(schema)를 사용한다.
-- 접속기의 Connection Name은 'NEWBIE'로 지정
-- Parameters의 Username은 'newbie'로 지정(계정명)
